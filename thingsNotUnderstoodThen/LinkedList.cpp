...
// functions: show and compare
class Data
{
public:
    Data(int newVal):value(newVal) {}
    ~Data() {}
    // decides whether a particular Data goes onward or stops there
...
/**
 * @brief Compare is used to decide where in the list a particular object belongs.
 * 
 * @param otherData A 比 B 小/大/同 中的B
 * @return int returns relative position
 *         {0, 1, 2} 分别对应 小/大/同
 */
...
// we cover templates.
// 和典型的数据结构一样 链表节点的数据成员包括了 {data, next}
...
/**
 * @brief Construct a new Internal Node:: Internal Node object
 *        All the constructor does is to initialize
 * 
 * @param newData member: the pointer to the data itself
 * @param newNext member: points to next node in the linked list
 */
...
/**
 * @brief The meat of the list.
 *        When you put a new object into the list, it is passed to the node 
 *        which figures out where it goes and inserts it into the list
 * 
 * @param otherData 需要插入的数据
 * @return Node* 当数据otherData不大于当前数据时，完成插入并返回指向otherData所在节点的指针
 *               否则将otherData向后插入，并返回指向当前节点的this指针
 */
...
/**
 * @brief If data comes to me, it must be inserted before me
 *        as I am the tail and NOTHING comes after me
 * 
 * @param data 需要插入的数据
 * @return Node* 完成插入并返回指向otherData所在节点的指针
 */
...
/**
 * @brief Nothing comes before the head so just pass the data on to the next node
 * 
 * @param data 需要插入的数据 被头节点委托给后继节点
 * @return Node* 返回指向当前节点的this指针
 */
...
/**
 * @brief Delegate, delegate, delegate
 *        Call my member head to insert() the data
 * 
 * @param pData 需要插入的数据
 */
